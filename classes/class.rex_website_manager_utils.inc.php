...
class rex_website_manager_utils {
	...
	public static function getSettingsFile() {
		...
		$dataDir = WEBSITE_MANAGER_DATA_DIR;
	...
	public static function replaceSettings($settings) {
		...
		// type conversion
		foreach ($REX['WEBSITE_MANAGER_SETTINGS'] as $key => $value) {
			if (isset($settings[$key])) {
				$settings[$key] = self::convertVarType($value, $settings[$key]);
			}
		}

		$REX['WEBSITE_MANAGER_SETTINGS'] = array_merge((array) $REX['WEBSITE_MANAGER_SETTINGS'], $settings);
	}
	...
	public static function checkDir($dir) {
		...
		if (!@is_dir($path)) {
			if ($REX['REDAXO']) {
				return $I18N->msg('website_manager_install_make_dir', $dir);
			}
		} elseif (!@is_writable($path . '/.')) {
			if ($REX['REDAXO']) {
				return $I18N->msg('website_manager_install_perm_dir', $dir);
			}
		}
